MyApplication.controller("FriendController", function($scope, $http, $location,
	...
	function showFriendList() {
		...
		$http.get(
				'http://localhost:8083/LChat_Middleware/showFriendList/'
	...
	function showPendingFriendList() {
		...
		$http.get(
				'http://localhost:8083/LChat_Middleware/showPendingFriendRequest/'
						+ loginname).then(function(response) {
	...
	function showSuggestedFriendList() {
		...
		$http.get(
				'http://localhost:8083/LChat_Middleware/showSuggestedFriendList/'
						+ loginname).then(function(response) {
	...
	$scope.sendFriendRequest = function(loginname1) {
		...
		$http.post('http://localhost:8083/LChat_Middleware/sendFriendRequest',
	...
	$scope.unfriend = function(friendId) {
		...
		$http.get(
				'http://localhost:8083/LChat_Middleware/deleteFriendRequest/'
	...
	$scope.accept = function(friendId) {
		...
		$http.get(
				'http://localhost:8083/LChat_Middleware/acceptFriendRequest/'
						+ friendId).then(function(response) {
