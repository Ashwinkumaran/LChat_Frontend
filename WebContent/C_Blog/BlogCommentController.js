MyApplication.controller("BlogCommentController",function($scope,$rootScope,$location,$http,$route)
{
	...
	function listBlogComment(){
		...
		$http.get('http://localhost:8083/LChat_Middleware/listBlogComment/'+$rootScope.blogId)
	...
	function getABlog()
	{
		...
		console.log($rootScope.blogId);
		$http.get('http://localhost:8083/LChat_Middleware/getBlog/'+$rootScope.blogId)
		.then(function(response){
	...
	$scope.addcomment=function(){
		...
		$http.post('http://localhost:8083/LChat_Middleware/addBlogComment/',$scope.blogComment)
		.then(function(response){
			console.log("Success");
		});
	}
	$scope.delete =function(commentId){
		...
		$http.get('http://localhost:8083/LChat_Middleware/deleteComment/'+commentId)
		.then(function(response)
				{
