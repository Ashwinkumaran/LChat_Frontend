MyApplication.controller("JobController",function($scope,$http,$location,$route,$rootScope)
	...
	function listJob() {
		...
		$http.get('http://localhost:8083/LChat_Middleware/showAllJob')
	...
	$scope.addJob = function() {
		...
		alert("Job Added");
		$http.post('http://localhost:8083/LChat_Middleware/addJob',
	...
	$scope.delete = function(jobid) {
		...
		alert("Job Deleted");
		$http.get(
				'http://localhost:8083/LChat_Middleware/deleteJob/'
	...
	function getJob() {
		...
		$http.get('http://localhost:8083/LChat_Middleware/getJob/'+$rootScope.jobid)
				.then(function(response) {
					$scope.jobdata = response.data;
	...
	$scope.apply = function(jobid) {
		...
		$http.post(
				'http://localhost:8083/LChat_Middleware/applyJob',$scope.applyjobdata
						).then(function(response) {
							$scope.reload;
							console.log($scope.applyjobdata);
							ListAppliedJob();
	...
	function ListAppliedJob() {
		...
		$http.get('http://localhost:8083/LChat_Middleware/listAppliedJobs/'+loginname)
				.then(function(response) {
